...
namespace GodSharp.Sockets
{
    public sealed class TcpClient : NetBase<ITcpConnection, NetClientEventArgs<ITcpConnection>>, ITcpClient, IDisposable
    {
        ...
        public TcpClient(string remoteHost, int remotePort, int localPort = 0, string localHost = null, int connectTimeout = 3000, string name = null, int id = 0)
        {
            try
            {
                TcpClientOptions options = new TcpClientOptions() { ConnectTimeout = 3000 , Id = id, Name = name };
